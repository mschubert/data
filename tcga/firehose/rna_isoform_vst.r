...
archive_regex = "RSEM_isoforms__data.Level_3.*\\.tar(\\.gz)?$"
...
file2expr = function(fname, ids="hgnc", quiet=FALSE) {
    ...
    re = re[-1, re[1,] %in% c("isoform_id", "raw_count")]
    mat = data.matrix(re[,-1])

    rownames(mat) = re$isoform_id
...
rna_isoform_vst = function(regex=archive_regex, dir=util$data_dir) {
